...
namespace RTD266xFlash
{
    public static class FirmwareModifier
    {
        ...
        public static Firmware DetectFirmware(byte[] firmware, Forms.FormProcessing procFrm = null)
        {
            Firmware detectedFirmware = null;

            for (int x = 0; x < Firmware.KnownFirmwares.Length; x++)
            {
                Firmware fw = Firmware.KnownFirmwares[x];
                if (procFrm != null)
                {
                    procFrm.progressInfo.Text = "Searching for matching preset (" + x + " in " + Firmware.KnownFirmwares.Length + ")...";
                }
        ...
        public static Firmware FindMatchPattern(byte[] firmware, Forms.FormProcessing procFrm = null) // basically a faster detection algorithm where it just checks the offsets and data (making a hash is slow)
        {
            Firmware detectedFirmware = null;
            BinaryReader br = new BinaryReader(new MemoryStream(firmware));

            for (int x = 0; x < Firmware.KnownFirmwares.Length; x++)
            {
                Firmware fw = Firmware.KnownFirmwares[x];
                foreach (HashSkip hs in fw.HashInfo.SkippedSections)
                {
                    for (int y = 0; y < FirmwareAnalyzer._patterns.Length;y++)
                    {
                        if (procFrm != null)
                        {
                            procFrm.progressInfo.Text = "Comparing Pattern " + y + "/" + FirmwareAnalyzer._patterns.Length + " of firmware " + x + "/" + Firmware.KnownFirmwares.Length;
                        }
                        FirmwarePattern fwp = FirmwareAnalyzer._patterns[y];
                        br.BaseStream.Position = hs.Offset;
                        byte[] arr = br.ReadBytes(fwp.Pattern.Length);
                        if (fwp.Pattern != arr) { break; }
                        if (y == FirmwareAnalyzer._patterns.Length - 1) { detectedFirmware = fw; }
                    }
                    if (detectedFirmware != null) { break; }
                }
                if (detectedFirmware != null) { break; }
            }
