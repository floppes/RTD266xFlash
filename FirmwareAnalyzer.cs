...
namespace RTD266xFlash
{
    public class FirmwareAnalyzer
    {
        ...
        public static FirmwarePattern[] _patterns =
        {
        ...
        public static Firmware AnalyzeFirmware(byte[] firmware)
        {
            ...
            {
                if (MessageBox.Show("The firmware given has a different length than expected\n  Expected Length: 0x80000\n\nFirmware Length: 0x" + firmware.Length.ToString("X5"), "Unexpected Length", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.Cancel)
                {
                    return null;
                }
            }
            ...
            if (offsets.Length >= 1)
            {
                logoOffset = Properties.Settings.Default.UseLatestFound ? offsets[offsets.Length - 1] : offsets[0];
            }
            else
            {
                if (MessageBox.Show("Could not find logo in firmware!", "Analysis Failed!", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.Cancel)
                {
                    return null;
                }
            }
            ...
            if (offsets.Length >= 1)
            {
                hdmiStringOffset = Properties.Settings.Default.UseLatestFound ? Properties.Settings.Default.UseLatestFound ? offsets[offsets.Length - 1] : offsets[0] : offsets[0];
            }
            else
            {
                if (MessageBox.Show("Could not find 'HDMI' pop-up in firmware!", "Analysis Failed!", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.Cancel)
                {
                    return null;
                }
            }
            ...
            if (offsets.Length >= 1)
            {
                adjustBackgroundColorOffset = Properties.Settings.Default.UseLatestFound ? offsets[offsets.Length - 1] : offsets[0];
            }
            else
            {
                if (MessageBox.Show("Could not find Background Colors in firmware!", "Analysis Failed!", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.Cancel)
                {
                    return null;
                }
            }
            ...
            if (offsets.Length >= 1)
            {
                showNoteOffset = Properties.Settings.Default.UseLatestFound ? offsets[offsets.Length - 1] : offsets[0];
            }
            else
            {
                if (MessageBox.Show("Could not find note in firmware!", "Analysis Failed!", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.Cancel)
                {
                    return null;
                }
            }
            ...
            if (offsets.Length >= 1)
            {
                paletteOffset = Properties.Settings.Default.UseLatestFound ? offsets[offsets.Length - 1] : offsets[0];
            }
            else
            {
                if (MessageBox.Show("Could not find color palette in firmware!", "Analysis Failed!", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.Cancel)
                {
                    return null;
                }
            }
            ...
            if (offsets.Length >= 1)
            {
                noSignalOffset = Properties.Settings.Default.UseLatestFound ? offsets[offsets.Length - 1] : offsets[0];
            }
            else
            {
                if (MessageBox.Show("Could not find 'No Signal' pop-up in firmware!", "Analysis Failed!", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) == DialogResult.Cancel)
                {
                    return null;
                }
            }
            ...
            HashInfo hashInfoTemp = new HashInfo(0, firmware.Length, string.Empty, hashSkips.ToArray()),
                     hashInfo = new HashInfo(0, firmware.Length, hashInfoTemp.GetHash(firmware), hashSkips.ToArray());
